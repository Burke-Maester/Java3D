...
public class Prism {
	...
	public Prism(double x, double y, double z, double width, double length, double height, Color c) {
		...
		Polys[1] = new DPolygon(
            new double[] { x, x,          x + width,  x + width },
            new double[] { y, y,          y,          y         },
		...
		Polys[2] = new DPolygon(
            new double[] { x + width, x + width,  x + width  },
            new double[] { y,         y,          y + length },
            new double[] { z,         z + height, z + height },
            c,
            false
        );
		Screen.DPolygons.add(Polys[2]);
		Polys[3] = new DPolygon(
            new double[] { x,          x,          x + width,  x + width  },
	...
	void Remove() {
		for(int i = 0; i < 5; i ++) { Screen.DPolygons.remove(Polys[i]); }
